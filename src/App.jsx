...
import changeBgImg from './Functions/UpdateBgImg.js';
...
export default function App() {
    ...
    const updateWeather = async () => {
        try {
            ...
            let api = `https://api.weatherapi.com/v1/forecast.json?key=d3da611f4f204186868114349241802&q=${city}&days=1&aqi=yes&alerts=no`;
            setSpin(true);
    ...
    return (
        <>
            <div className=" py-3 px-5 bg-[#0e0d0d] fixed w-full z-10 top-0 xl:hidden">
                <div className="relative ">
                    ...
                    <input type="text" id='inputa' className="block w-full rounded-lg border border-gray-300 p-3 pl-11 text-sm text-gray-900" placeholder="Search..." required />
                    <button onClick={updateWeather} className="absolute bottom-1.5 right-2.5 rounded-xl bg-blue-950 px-4 py-2 text-sm font-medium text-white hover:bg-blue-800 active:bg-blue-950">Search</button>
                </div>
            </div>
            ...
            <div className='flex flex-col xl:flex-row 2xl:space-x-8 relative top-20 xl:top-0'>
                ...
                <div className=' w-1/3 2xl:w-1/4 2xl:ml-12 h-screen hidden  xl:flex justify-center items-center border-r-[0.3px] border-gray-800'>
                    <div className='text-center rounded-xl bg-slate-200  h-fit py-20 flex flex-col justify-center items-center shadow-sm shadow-slate-200'>
                        <h1 className=' font-bold font-sans text-2xl'>Enter Your City</h1>
                        <input id='inputb' onKeyPress={handleKeyPress} className=' border-2 outline-none rounded-md border-slate-400 bg-gray-100 m-5 h-10 w-[23rem] pl-3 text-lg' type="text" placeholder='Eg. India, New Delhi, London' />
                        <br />
                        ...
                        <p className=' bg-slate-200 w-8 relative bottom-[0.9rem] font-semibold text-lg'>Or</p>
                        <button className='bg-gray-400  hover:bg-gray-400 active:bg-gray-500  flex justify-center rounded-md font-semibold px-3 py-1 w-[23rem]'
