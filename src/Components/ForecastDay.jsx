...
export default function ForecastDay({city}) {
    ...
    const updateForecast = async () => {
        try {
            let api = `https://api.weatherapi.com/v1/forecast.json?key=d3da611f4f204186868114349241802&q=${city}&days=6&aqi=yes&alerts=no`;
        ...
        <>
            <div className='flex  overflow-x-scroll border-b-[0.3px] pb-3 border-gray-800 ml-2'>
                ...
                {currentForecast.map(current => {
                    return (
                        <div key={current.time} className=' ml-1 md:ml-2 flex flex-col justify-between bg-white rounded-2xl shadow-sm shadow-gray-200 m-2 px-4 py-2 text-center '>
